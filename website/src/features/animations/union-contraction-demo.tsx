...
export function UnionContractionDemo() {
  ...
  const renderSet = (label: string, nodes: number[]) => {
    ...
    const setInner = nodes.length > 0 ? `\{${nodes.sort().join(", ")}\}` : "empty";
    return (
      <div
      ...
      >
        ...
        <div className="min-h-[1.5rem] break-all font-mono text-xs leading-relaxed" style={{ color: `${color}ee` }}>
          <FormatMathText text={String.raw`$\{${setInner}\}$`} />
        </div>
      </div>
    );
  };
    ...
    <Card>
      <div className="grid gap-8 lg:grid-cols-[0.85fr_1.15fr]">
        <div className="space-y-5">
          <SurfaceTitle
            title="Animation A · Union contraction in duplex networks"
            body="The budgets $|D_1|$ and $|D_2|$ stay fixed. What changes is which nodes play the driver role and how much the two layers overlap."
          />
          <div className="grid gap-3 sm:grid-cols-2">
            <StatCard label="$|D_1|$" value={state.d1.length} color={APP_CONFIG.colors.layer1.driver} hint="Layer 1 budget stays fixed" />
            <StatCard label="$|D_2|$" value={state.d2.length} color={APP_CONFIG.colors.layer2.driver} hint="Layer 2 budget stays fixed" />
            <StatCard label="$|U|$" value={unionSize} color={APP_CONFIG.colors.default.gray} hint="Unique actuated nodes" />
            <StatCard label="$\Delta$" value={delta} color={APP_CONFIG.colors.default.gray} hint="Difference mass" />
          </div>
          <div className="text-sm leading-7 text-ink/72">
            <FormatMathText text={state.explanation} />
          </div>
        ...
        <div className="rounded-[26px] border border-ink/8 bg-gradient-to-br from-white to-mist/80 p-5">
          ...
          <div className="space-y-4">
            <div>
              <div className="mb-2 text-sm font-medium" style={{ color: APP_CONFIG.colors.layer1.driver }}>Layer 1 drivers</div>
                ...
                {universe.map((node) => (
                  <div
                    ...
                    style={state.d1.includes(node)
                      ...
                      : { borderColor: 'rgba(22,33,43,0.1)', backgroundColor: 'white', color: 'rgba(22,33,43,0.45)' }}
                  >
                    {node}
                  </div>
                ))}
            ...
            <div>
              <div className="mb-2 text-sm font-medium" style={{ color: APP_CONFIG.colors.layer2.driver }}>Layer 2 drivers</div>
              <div className="flex flex-wrap gap-3">
                {universe.map((node) => (
                  <div
                    key={`l2-${node}`}
          ...
          <div className="mt-6 space-y-4 rounded-[28px] border border-ink/8 bg-white/90 p-5 shadow-sm">
            <div className="flex items-center justify-between px-1">
              <div className="text-sm font-bold text-ink">Partition set view</div>
              <div className="text-[10px] font-medium text-ink/40 uppercase tracking-widest">Refined status</div>
            </div>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    index === itemIndex
                      ...
                      : "bg-white text-ink/60 border border-ink/10 hover:border-ink/20 hover:text-ink"
                  )}
                >
                  {item.label}
                </button>
